...
def knit_swatch(knit_graph: Knit_Graph, out_put_name: str = "swatch_instructions"):
    ...
    knit_graph_built = knit_script_to_knitout_to_dat("Swatch_Knitter.ks", f"{out_put_name}", dat_name=f"{out_put_name}.dat", pattern_is_filename=True,
                                                     python_variables={"swatch": knit_graph, "courses": courses}, visualize_instruction_graph=True)
