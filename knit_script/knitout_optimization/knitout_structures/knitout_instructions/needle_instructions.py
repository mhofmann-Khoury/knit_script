...
class Knit_Instruction(Needle_Instruction):
    ...
    def __init__(self, needle: Needle, direction: str, cs: Carrier_Set):
        super().__init__(Instruction_Type.Knit, needle, direction=Pass_Direction.get_direction(direction), carrier_set=cs)


class Tuck_Instruction(Needle_Instruction):

    def __init__(self, needle: Needle, direction: str, cs: Carrier_Set):
        super().__init__(Instruction_Type.Tuck, needle, direction=Pass_Direction.get_direction(direction), carrier_set=cs)


class Split_Instruction(Needle_Instruction):

    def __init__(self, needle: Needle, direction: str, n2: Needle, cs: Carrier_Set):
        super().__init__(Instruction_Type.Split, needle, direction=Pass_Direction.get_direction(direction), needle_2=n2, carrier_set=cs)


class Drop_Instruction(Needle_Instruction):

    def __init__(self, needle: Needle):
        super().__init__(Instruction_Type.Split, needle)


class Amiss_Instruction(Needle_Instruction):

    def __init__(self, needle: Needle):
        super().__init__(Instruction_Type.Amiss, needle)


class Xfer_Instruction(Needle_Instruction):

    def __init__(self, needle: Needle, n2: Needle):
        super().__init__(Instruction_Type.Xfer, needle, needle_2=n2)


class Miss_Instruction(Needle_Instruction):

    def __init__(self, needle: Needle, direction: str, cs: Carrier_Set):
        super().__init__(Instruction_Type.Miss, needle, direction=Pass_Direction.get_direction(direction), carrier_set=cs)
