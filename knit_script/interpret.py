...
def knit_script_to_knitout_to_dat(pattern: str, knitout_name: str, dat_name: Optional[str] = None, pattern_is_filename: bool = False,
                                  optimize=True, visualize_instruction_graph: bool = False) -> Knit_Graph:
    """
    Processes a knit script pattern into knitout and a dat file for shima seiki machines and returns the resulting knit graph from the operations
    :param: pattern: the knit script pattern or a file containing it
    ...
    """
    interpreter = Knit_Script_Interpreter()
    _, knit_graph = interpreter.write_knitout(pattern, knitout_name, pattern_is_filename, optimize=optimize, visualize_instruction_graph=visualize_instruction_graph)
