...
class Attribute_Accessor_Expression(Expression):
    ...
    def __init__(self, parent_path: Expression,
    ...
    def __str__(self):
        return f"{self._parent_path()}.{self.attribute}"
    ...
    def evaluate(self, context: Knit_Script_Context) -> Any:
        ...
        if len(self.parent) == 1: # one parent expression
            ...
            parent_source_str = self._parent_path()
            try:
                parent = context.variable_scope[parent_source_str] # stored from an import statement
            except KeyError as e:
                # recursively process parent string
        ...
        if isinstance(self.attribute, Variable_Expression):  # variable name, access directly from parent instance
            ...
            if isinstance(parent, Machine_State):
                ...
                assert False, f"Cannot access {self.attribute} from {parent}"
