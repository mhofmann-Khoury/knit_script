...
class Parsing_Exception(Knit_Script_Exception):
    ...
    Attributes:
        syntax_error (ParseError): The original ParseError from the parglare parser.
    ...
    def __init__(self, parglare_syntax_error: parglare.exceptions.SyntaxError) -> None:
        ...
        Args:
            parglare_syntax_error (ParseError): The original ParseError from the parglare parser containing the raw parsing failure information.
        """
        self.syntax_error: parglare.exceptions.SyntaxError = parglare_syntax_error
        self.error_location: Location = self.syntax_error.location
        ...
        self._expected: str = f"Expected: {[t.name for t in self.syntax_error.symbols_expected]}"
